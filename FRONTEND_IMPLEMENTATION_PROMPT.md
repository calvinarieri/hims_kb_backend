# Frontend Implementation Prompt: Role-Based Access Control (RBAC) & Staff Management

Use this document as an actionable prompt for implementing the frontend user interface for **Role & Permissions Management**, **Staff Account Lifecycle Management**, and **Granular Permission Guards** in your React/Vue/Svelte frontend application.

---

## 🎯 Task Overview
Build the frontend pages and components to interface with the HIMS KB backend API for:
1. **Roles & Permissions Management**: Create, edit, and delete user roles with custom feature permission checkboxes grouped by category.
2. **Staff Account Lifecycle Management**: Add new staff members (triggering automated email credentials), edit details, reset passwords, and dismiss/deactivate staff accounts.
3. **Permission Guards & UI Hiding**: Conditionally render navigation items, action buttons, and routes based on the current user's role permissions.

---

## 📡 Backend API Endpoints Reference

### 1. Roles & Permissions (`/auth/roles/`)
- `GET /auth/roles/available-permissions/`: Returns list of system feature permissions grouped by category.
  ```json
  {
    "status_code": 200,
    "data": [
